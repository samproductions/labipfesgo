...
import { ViewState, AdminTab, Member, Project, Lab, CalendarEvent, FeedPost, UserProfile, DirectMessage, MembershipSettings, AttendanceRecord } from './types';
...
const App: React.FC = () => {
  ...
  // SINCRONIZAÇÃO DE PERMISSÕES MASTER E MEMBRO
  useEffect(() => {
    if (currentUser && isAuthenticated && !isSyncing) {
      ...
      let newRole: UserProfile['role'] = isAdmin ? 'admin' : (memberData ? 'member' : 'student');
  ...
  if (!isAuthenticated || !currentUser) {
    return <Auth onLogin={(u) => { setCurrentUser(u); setIsAuthenticated(true); localStorage.setItem('lapib_auth_status', 'true'); localStorage.setItem('lapib_user', JSON.stringify(u)); }} />;
  }
  ...
  return (
    <Layout activeView={view} onNavigate={setView} user={currentUser} onLogout={() => { setIsAuthenticated(false); setCurrentUser(null); localStorage.clear(); setView('home'); }} logoUrl={appLogo} unreadCount={unreadCount}>
      ...
      {view === 'feed' && <Feed posts={posts} user={currentUser} onNavigateToLogin={() => setView('home')} onPostsUpdate={() => {}} onSavePost={(p) => CloudService.savePost(p)} />}
      ...
      {view === 'projects' && (
        <div className="max-w-6xl mx-auto animate-in fade-in duration-500 text-left">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            ...
            {projects.length === 0 && <div className="col-span-full py-20 text-center opacity-30 font-black uppercase tracking-widest text-xs">Nenhum projeto protocolado</div>}
          </div>
        </div>
      )}

      {view === 'members' && <Members members={members} />}
      {view === 'ingresso' && <Membership settings={membershipSettings} onApply={(d) => {}} />}
      {view === 'profile' && <Profile user={currentUser} onUpdateUser={(u) => { setCurrentUser(u); CloudService.saveUser(u); }} />}

      {/* ESPAÇO ACADÊMICO (RESTRITO) */}
      ...
      {view === 'messages' && (
        <MessageCenter 
          ...
          members={members}
          onSendMessage={async (txt, fileData, receiverId) => {
            const newMsg: DirectMessage = { id: Date.now().toString(), senderId: currentUser.id, senderName: currentUser.fullName, receiverId: receiverId, message: txt, fileUrl: fileData?.data, fileName: fileData?.name, timestamp: new Date().toISOString(), read: false };
            await CloudService.saveMessage(newMsg);
          }} 
        />
      )}
      ...
      {view === 'admin' && (
        <div className="max-w-6xl mx-auto animate-in zoom-in-95 duration-500 text-left">
           ...
           <div className="bg-white rounded-[5rem] p-16 shadow-2xl border border-slate-50 min-h-[65vh] max-w-5xl mx-auto flex flex-col">
              {adminTab === 'events' && <div className="w-full space-y-10"><div className="flex items-center justify-between mb-8"><h3 className="text-3xl font-black text-slate-800 uppercase tracking-tighter">Cronograma</h3><button onClick={() => setShowEventForm(true)} className="bg-[#055c47] text-white px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl hover:bg-black transition-all">Novo Evento</button></div><div className="grid grid-cols-1 md:grid-cols-2 gap-4">{events.map(e => (<div key={e.id} className="p-6 bg-slate-50 rounded-3xl border border-slate-100 flex items-center justify-between"><div><p className="text-xs font-black text-slate-800 uppercase">{e.title}</p></div><button onClick={() => CloudService.deleteEvent(e.id)} className="w-10 h-10 rounded-xl bg-red-50 text-red-500 flex items-center justify-center"><i className="fa-solid fa-trash-can text-xs"></i></button></div>))}</div></div>}
              {adminTab === 'attendance' && <AdminAttendance attendances={attendances} events={events} members={members} onAddAttendance={(r) => CloudService.saveAttendance(r)} onDeleteAttendance={(id) => CloudService.deleteAttendance(id)} />}
              {adminTab === 'projects' && <div className="w-full space-y-10"><div className="flex items-center justify-between mb-8"><h3 className="text-3xl font-black text-slate-800 uppercase tracking-tighter">Projetos</h3><button onClick={() => setShowProjectForm(true)} className="bg-[#055c47] text-white px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl hover:bg-black transition-all">Novo Projeto</button></div><div className="grid grid-cols-1 md:grid-cols-2 gap-4">{projects.map(p => (
                <div key={p.id} className="p-6 bg-slate-50 rounded-3xl border border-slate-100 flex items-center justify-between">
                  <div className="flex items-center gap-4"><img src={p.imageUrl} className="w-12 h-12 rounded-xl object-cover border" alt="" /><div><p className="text-xs font-black text-slate-800 uppercase">{p.title}</p></div></div>
                  <button onClick={() => CloudService.deleteProject(p.id)} className="w-10 h-10 rounded-xl bg-red-50 text-red-500 flex items-center justify-center"><i className="fa-solid fa-trash-can text-xs"></i></button>
                </div>
              ))}</div></div>}
              {adminTab === 'labs' && <div className="w-full space-y-10"><div className="flex items-center justify-between mb-8"><h3 className="text-3xl font-black text-slate-800 uppercase tracking-tighter">Laboratórios</h3><button onClick={() => setShowLabForm(true)} className="bg-[#055c47] text-white px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl hover:bg-black transition-all">Novo Núcleo</button></div><div className="grid grid-cols-1 md:grid-cols-2 gap-4">{labs.map(l => (
                <div key={l.id} className="p-6 bg-slate-50 rounded-3xl border border-slate-100 flex items-center justify-between">
                  ...
                  <button onClick={() => CloudService.deleteLab(l.id)} className="w-10 h-10 rounded-xl bg-red-50 text-red-500 flex items-center justify-center"><i className="fa-solid fa-trash-can text-xs"></i></button>
                </div>
              ))}</div></div>}
              {adminTab === 'members' && <AdminMembers members={members} onAddMember={(m) => CloudService.saveMember(m)} onDeleteMember={(id) => {}} onToggleAccess={(id) => {
                const member = members.find(m => m.id === id);
                if(member) CloudService.updateMemberAccess(id, !member.acessoLiberado);
              }} currentUser={currentUser} />}
              {adminTab === 'settings' && <AdminMembership settings={membershipSettings} onUpdateSettings={(s) => CloudService.saveSettings(s)} onLogoUpload={(e) => {
                const f = e.target.files?.[0];
                if(f) {
                  const r = new FileReader();
                  r.onloadend = () => { const data = r.result as string; setAppLogo(data); localStorage.setItem('lapib_logo', data); };
                  r.readAsDataURL(f);
                }
              }} />}
           </div>
      ...
      {showProjectForm && <ProjectFormModal onClose={() => setShowProjectForm(false)} onSave={(p) => { CloudService.saveProject(p); setShowProjectForm(false); }} />}
      {showEventForm && <EventFormModal onClose={() => setShowEventForm(false)} onSave={(e) => { CloudService.saveEvent(e); setShowEventForm(false); }} />}
      {showLabForm && <LabFormModal onClose={() => setShowLabForm(false)} onSave={(l) => { CloudService.saveLab(l); setShowLabForm(false); }} />}
      {enrollModal.active && <EnrollmentModal activity={{ id: enrollModal.event?.id || enrollModal.lab?.id || '', title: enrollModal.event?.title || enrollModal.lab?.title || '', description: enrollModal.event?.desc || enrollModal.lab?.desc || '' }} event={enrollModal.event} onClose={() => setEnrollModal({ active: false })} onSubmit={(d) => {}} />}
    </Layout>
  );
};
